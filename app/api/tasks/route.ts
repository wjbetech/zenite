...
export async function GET() {
  // find userId; if unauthenticated return empty array for local/dev rendering
  const { userId } = await auth();
  if (!userId) {
    // For development and ease of UI testing, return an empty list instead of 401.
    // This keeps the client rendering predictable while still protecting write operations.
    return NextResponse.json([]);
  }
